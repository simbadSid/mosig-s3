...
void Architecture(NumNeurons, NbreCouche)
long int  *NumNeurons, *NbreCouche;
{
   long int  i;
   ...
   {
      printf("Number of neurons of the %ld-th layer\n", i);
	  scanf("%ld",&NumNeurons[i]);
   }   
}
...
void FileScanMltCls(char *filename, long int *m, long int *d, long int *K)
{
   FILE *fd;
   long int  i=0, ch, prev='\n', dim=0, lines=0,cls;
   char *str, *ptr, sep[]=": \t";
...
void ChargeFicBaseMltClass(char *FicB, DATA *D)
{
   long int i,j,c;
   FILE *fd1;
